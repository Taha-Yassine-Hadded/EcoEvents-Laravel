...
@section('title', 'Modifier une Catégorie - Echofy')

@section('content')
    <!-- Content Header -->
    <div class="content-header">
        <h1 class="page-title">Modifier la Catégorie: {{ $category->name }}</h1>
        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="card-title">Formulaire de Modification de Catégorie</h6>
                </div>
                <div class="card-body">
                    @if (session('error'))
                    ...
                    <div class="form-header">
                        ...
                        <p>Mettez à jour les informations de cette catégorie</p>
                    </div>

                    <form id="categoryForm" method="POST" action="{{ route('admin.categories.update', $category) }}">
                        @csrf
                        @method('PUT')
                        <div class="form-section">
                            ...
                            <div class="form-group">
                                ...
                                <textarea id="description" name="description" class="form-control" rows="4" placeholder="Décrivez brièvement la catégorie...">{{ old('description', $category->description) }}</textarea>
    ...
    <script>
        ...
        function setupCharacterCounters() {
            ...
            counters.forEach(({ input, counter, max }) => {
                ...
                const initialCount = inputElement.value.length;
            ...
            .catch(error => {
                if (error.status === 422 && error.data && error.data.messages) {
                    Object.keys(error.data.messages).forEach(field => {
                        ...
                        if (errorElement) {
                            errorElement.textContent = error.data.messages[field][0];
                            errorElement.style.display = 'block';
