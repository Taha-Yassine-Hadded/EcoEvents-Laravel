# server.py - DÉTECTION ANGLAIS + NRC OPTIMISÉ
import os
import re
import logging
import time
...
app = FastAPI(title="EMOLEX + NRC + English Fixed")
...
class LexiconAnalyzer:
    def __init__(self):
        ...
        self.french_keywords = set([
            'j', 'je', 'tu', 'il', 'elle', 'nous', 'vous', 'ils', 'elles',
            'le', 'la', 'les', 'de', "d'", 'à', 'au', 'aux', 'et', 'ou',
            'mais', 'donc', 'or', 'car', 'triste', 'génial', 'joyeux',
            'maladie', "j'adore", 'projet', 'cest', 'est', 'suis'
        ])
        self.tunisian_keywords = set(['ana', 'barcha', 'farhèn', 'nebki', 'mridha',
                                     'khayef', 'khouya', 'zwin', 'yallah', 'kif'])
        self.english_keywords = set(['i', 'you', 'he', 'she', 'it', 'we', 'they',
                                   'the', 'a', 'an', 'and', 'or', 'but', 'in',
                                   'on', 'at', 'to', 'for', 'of', 'with', 'idiot',
                                   'lying', 'horrible', 'worry', 'threaten'])
    ...
    def detect_language_first(self, text: str) -> str:
        ...
        words = re.findall(r'\b[a-zA-Zàâäéèêëîïôöùûüçأ-ي]{2,}\b', text_lower)

        # Compteurs
        tunisian_count = sum(1 for w in words if w in self.tunisian_keywords)
        french_count = sum(1 for w in words if w in self.french_keywords)
        english_count = sum(1 for w in words if w in self.english_keywords or re.match(r'^[a-z]{4,}$', w))

        # Priorité
        if tunisian_count > 0:
            return 'tunisian'
        if french_count > english_count * 0.7:
            return 'french'
        if english_count > french_count * 0.5 or english_count > 2:
            return 'english'

        # Fallback EMOLEX
        emolex_matches = self._quick_emolex_check(words)
        if emolex_matches.get('french', 0) > emolex_matches.get('english', 0):
            return 'french'

        return 'english'  # Default anglais pour NRC

    def _quick_emolex_check(self, words: List[str]) -> Dict:
        """🔍 Vérification rapide EMOLEX"""
        matches = defaultdict(int)
        for word in words[:10]:  # Top 10 mots
    ...
    def analyze_text_matches(self, text: str, detected_lang: str) -> Dict:
        """🔍 ANALYSE PAR LANGUE"""
        words = re.findall(r'\b[a-zA-Zàâäéèêëîïôöùûüçأ-ي]{2,}\b', text.lower())
        lang_matches = defaultdict(lambda: {'count': 0, 'words': set(), 'emotions': Counter()})
        nrc_matches = Counter()
        tunisian_matches = []

        # 1. TUNISIAN (priorité)
        if detected_lang == 'tunisian' or 'tunisian' in detected_lang:
            for word in words:
                if word in self.tunisian_words:
                    emotions = self.tunisian_words[word]
                    tunisian_matches.append((word, emotions))
                    lang_matches['tunisian']['count'] += 1
                    lang_matches['tunisian']['words'].add(word)
                    for emo in emotions:
                        lang_matches['tunisian']['emotions'][emo] += 1

        # 2. ENGLISH NRC
        if detected_lang == 'english':
            english_words = [w for w in words if re.match(r'^[a-z]{3,}$', w)]
            for emotion, emotion_words in self.nrc_emotions.items():
                matches = sum(1 for w in english_words if w in emotion_words)
                nrc_matches[emotion] += matches
                # Ajouter comme matches
                lang_matches['english']['count'] += matches
                for w in english_words:
                    if w in emotion_words:
                        lang_matches['english']['words'].add(w)
                        lang_matches['english']['emotions'][emotion] += 1

        # 3. EMOLEX par langue détectée
        target_langs = [detected_lang]
        if detected_lang == 'unknown':
            target_langs = ['french', 'english']

        for lang in target_langs:
            for word in words:
                for emotion, langs in self.emolex_words.items():
                    if lang in langs and word in langs[lang]:
                        lang_matches[lang]['count'] += 1
                        lang_matches[lang]['words'].add(word)
                        lang_matches[lang]['emotions'][emotion] += 1

        # Best matches
        best_lang = detected_lang if detected_lang in lang_matches else max(lang_matches, key=lambda x: lang_matches[x]['count'], default='english')
        best_matches = lang_matches[best_lang]
        compound = self._calculate_compound(best_matches['emotions'], nrc_matches)
    ...
    def _calculate_compound(self, emolex_emotions: Counter, nrc_emotions: Counter) -> float:
        """📊 COMPOUND AVEC NRC"""
        ...
        total_pos = pos_emolex + pos_nrc
        total_neg = neg_emolex + neg_nrc
        total = total_pos + total_neg
        return (total_pos - total_neg) / max(total, 1)
    ...
    def get_dominant_emotion(self, analysis: Dict, detected_lang: str) -> str:
        ...
        if compound > 0:
            ...
            candidates = ['anger', 'negative', 'sadness']
            return max(candidates, key=lambda x: emotions.get(x, 0), default='anger')
        return 'neutral'
...
async def analyze_comment(request: CommentRequest):
    ...
    try:
        ...
        emotions_normalized[dominant_emotion] = min(emotions_normalized[dominant_emotion] + 0.3, 0.8)

        # Confiance
        matches = (len(analysis['tunisian_matches']) + analysis['best_matches']['count'] +
                  sum(analysis['nrc_matches'].values()))
